// Generated PersonList Component
...
let hooks = {}
try {
  const module = await import('./PersonList.hooks.js')
...
export default function PersonList() {
  ...
  try {
    ...
    console.warn('PersonList: Hook context not ready during navigation:', error.message)
    return html`
  ...
  const tableActions = [
    { label: 'Edit', onClick: (item) => window.location.href = `/people/${item.id}/edit` },
    ...
    <div class="person-list">
      ...
      <${Card}>
        <div class="flex justify-between items-center mb-4">
          <h1>All People</h1>
          <${Button} onClick=${() => window.location.href = '/people/new'}>
            Add Person
          </${Button}>
        </div>
        ...
        <${Table}
          data=${sortedData}
          columns=${[{"key":"createdAt","label":"CreatedAt"},{"key":"updatedAt","label":"UpdatedAt"},{"key":"name","label":"Name"},{"key":"email","label":"Email"},{"key":"role","label":"Role"},{"key":"department","label":"Department"},{"key":"userId","label":"UserId"}]}
